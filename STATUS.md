...
Beethoven is an engineering preview. It is suitable for trusted local runs and
...
|---|---|---|---|
| Claude | `runtime.kind: claude` | Working | Uses `@anthropic-ai/claude-agent-sdk`; Beethoven tools are exposed through an in-process SDK MCP server and are not always loaded. |
| Codex | `runtime.kind: codex` | Working | Uses `codex app-server` JSON-RPC; Beethoven tools are exposed as Codex dynamic tools. |
...
- Shared Beethoven tools live in `typescript/src/tools/` and are adapted per harness.
