...
`opencode run`). Workflows pick a harness via `runtime.kind` in the front
matter; everything else (tracker integration, workspace lifecycle,
orchestration, observability) is identical across harnesses.
...
- `model` (string, OPTIONAL)
  - Model identifier. Each harness uses its own naming
    (`claude-opus-4-7`, `gpt-5.4`, `gemini-2.5-pro`, `anthropic/claude-3-5-sonnet`).
  - When unset, the harness uses its own default.
- `effort` (string, OPTIONAL)
  - One of `low`, `medium`, `high`, `xhigh`, `max`.
  - Maps to the harness-native reasoning-effort knob (Claude `--effort`,
    Codex `effort` field, etc).
- `permission_mode` (string, OPTIONAL)
  - One of `default`, `acceptEdits`, `bypassPermissions`.
  - Harnesses that have richer per-tool policies (Codex's `approval_policy`)
    MAY further refine this via the per-harness sub-block.
- `cwd` (string, OPTIONAL)
  - Default: `"."`. Resolved relative to the workspace path at run time.
- `turn_timeout_ms` (integer, OPTIONAL)
  - Default: `3600000` (1 hour).
- `read_timeout_ms` (integer, OPTIONAL)
  - Default: `5000`.
- `stall_timeout_ms` (integer, OPTIONAL)
  - Default: `300000` (5 minutes). If `<= 0`, stall detection is disabled.
...
##### 5.3.6.1 `runtime.claude`

- `thinking_mode` (string, OPTIONAL)
  - One of `adaptive`, `enabled`, `disabled`.
- `thinking_budget_tokens` (positive integer, OPTIONAL)
  - Required when `thinking_mode = enabled`. Default `8000`.
- `executable` (string, OPTIONAL)
  - Override path to the `claude` binary. When unset, implementations SHOULD
...
- `sandbox_policy` (string, OPTIONAL)
  - One of `readOnly`, `workspaceWrite`, `dangerFullAccess`,
    `externalSandbox`. Default implementation-defined.
- `personality` (string, OPTIONAL)
  - Pass-through to the Codex `personality` field; supported values depend on
    the targeted model.

For other Codex-owned values, implementors SHOULD treat them as pass-through.
To inspect the installed Codex schema, run
`codex app-server generate-json-schema --out <dir>` and consult the
definitions referenced by `v2/ThreadStartParams.json` and
`v2/TurnStartParams.json`.

##### 5.3.6.3 `runtime.gemini`

- `executable` (string, OPTIONAL)
  - Override path to the `gemini` binary.
- `include_directories` (list of strings, OPTIONAL)
  - Forwarded to `--include-directories` (or the equivalent runtime-side
    `/directory add` chain).

V1 ships the Gemini harness as **experimental**: the headless protocol is
still maturing and field semantics may shift between Gemini CLI releases.

##### 5.3.6.4 `runtime.opencode`

- `executable` (string, OPTIONAL)
  - Override path to the `opencode` binary.
- `provider` (string, OPTIONAL)
  - Provider portion of opencode's `--model provider/model` (e.g.,
    `anthropic`, `openai`).
- `attach_url` (string, OPTIONAL)
  - When set, dispatch attaches to a long-lived `opencode serve` instance via
    `--attach <url>` instead of cold-starting a fresh process per dispatch.
- `resume_session` (string, OPTIONAL)
  - Pass-through to `--session <id>`.

##### 5.3.6.5 Normalized event stream

Regardless of harness, the orchestrator MUST consume a normalized event
stream. The minimal vocabulary:

- `session_started` — first event; carries the harness-native session id.
- `text_delta` — assistant text content (TextPart-shaped).
- `reasoning_delta` — reasoning/thinking content (Codex `item/reasoning/*`,
  Claude extended-thinking blocks).
- `tool_call` — agent invoked a tool (ToolCallPart-shaped: `toolName`,
  `toolCallId`, `input`).
- `tool_result` — tool produced a result (ToolResultPart-shaped: matches a
  prior `toolCallId`, includes `output` and `isError`).
- `tokens_updated` — cumulative + delta token counts. The cumulative SHOULD
  reflect "current context window size" (last server-side iteration), not
  billing-cumulative across iterations.
- `approval_requested` — out-of-band decision request (Codex
  `requestApproval` notifications). Harnesses without inline approvals MAY
  omit this entirely.
- `raw` — escape hatch for harness-specific events not yet normalized;
  carries the harness `kind` and the native event name.
...
6. Resolve harness skills symlink (see §9.2.1).

#### 9.2.1 Harness skills symlink
...
Some harnesses read the canonical path natively — `opencode` reads
`.agents/skills` directly, so no symlink is created when the harness
`skillsPath` already equals `.agents/skills`. The lifecycle treats this
as a no-op (`is_canonical_source`).

Algorithm:

1. Compute canonical source: `<workspace>/.agents/skills`.
2. Compute harness link path: `<workspace>/<harness.skillsPath>`.
   `skillsPath` is taken from the active harness adapter, which sources it
   from `runtime.{kind}.skills_path` if present, else the per-harness
   default in `DEFAULT_SKILLS_PATHS`.
3. If the canonical source does not exist, skip and emit a `skipped` log.
4. If the link path is occupied by a non-symlink (real directory or file),
   skip and emit a `skipped` log — operator intent is preserved.
5. If the link path is a symlink already pointing at the canonical source
   (resolved relatively), no-op.
6. Otherwise, create or replace the symlink with a relative target so the
   workspace can be moved or remounted under a different prefix.

This step is idempotent and runs before `before_run` so any pre-run hook
that consults skill content sees the linked layout. Implementations MUST
log the result (`linked` / `already_linked` / `skipped` / `failed`) at
debug level for operator visibility.
