...
9. Append a `Co-authored-by` trailer that identifies the active harness
   (e.g. `Claude <noreply@anthropic.com>` for the claude harness,
   `Codex <codex@openai.com>` for codex, etc.) unless the user explicitly
   requests a different identity.
...
Co-authored-by: <Harness> <harness-bot@example.com>
```

Replace `<Harness>` with the active runtime — e.g. `Claude <noreply@anthropic.com>`,
`Codex <codex@openai.com>`, `Gemini <noreply@google.com>`, or whatever
identity the operator has configured.
