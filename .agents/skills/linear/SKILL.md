...
name: linear
description: |
  Use the `linear_graphql` client tool (or a Linear MCP server) for raw
  Linear GraphQL operations such as comment editing and upload flows.
---

# Linear GraphQL

Use this skill for raw Linear GraphQL work during agent sessions.

## Primary tool

Use the `linear_graphql` client tool (or whichever Linear GraphQL access the
active harness exposes — Linear MCP server, injected tool, or
`LINEAR_API_KEY` + `curl`). It reuses the session's configured Linear auth.
