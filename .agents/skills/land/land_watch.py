...
# Configurable integration surface. Defaults match this repo's automated
# review setup (the `chatgpt-codex-connector` GitHub App emitting comments
# whose header starts with REVIEW_HEADER_PREFIX). Override via env vars when
# adopting a different reviewer integration.
REVIEW_BOT_LOGINS = {
...
REVIEW_HEADER_PREFIX = REVIEW_HEADER_PREFIX
...
def latest_review_request_at(comments: list[dict[str, Any]]) -> datetime | None:
    ...
    for comment in comments:
        if is_review_bot_user(comment.get("user", {})):
            continue
        body = comment.get("body") or ""
        if REVIEW_TRIGGER_PHRASE not in body:
            continue
...
def filter_review_bot_comments(
    comments: list[dict[str, Any]],
    review_requested_at: datetime | None,
) -> list[dict[str, Any]]:
    latest_bot_reply = latest_bot_reply_by_thread(comments)
    latest_issue_ack = latest_bot_issue_reply_time(comments)
    bot_comments = [c for c in comments if is_review_bot_user(c.get("user", {}))]
    ...
    for comment in bot_comments:
        ...
        if not is_threaded:
            ...
            if thread_root is not None:
                last_reply = latest_bot_reply.get(thread_root)
...
def is_review_bot_user(user: dict[str, Any]) -> bool:
    login = user.get("login") or ""
    return login in REVIEW_BOT_LOGINS


def is_bot_user(user: dict[str, Any]) -> bool:
    login = user.get("login") or ""
    if is_review_bot_user(user):
        return True
...
def is_bot_reply_body(body: str) -> bool:
    return body.startswith(BOT_REPLY_PREFIX)


def is_review_header_body(body: str) -> bool:
    return body.startswith(REVIEW_HEADER_PREFIX)


def latest_bot_issue_reply_time(
    comments: list[dict[str, Any]],
) -> datetime | None:
    latest: datetime | None = None
    for comment in comments:
        body = (comment.get("body") or "").strip()
        if not is_bot_reply_body(body):
            continue
        created_time = comment_time(comment)
        if created_time is None:
            continue
...
def filter_human_issue_comments(comments: list[dict[str, Any]]) -> list[dict[str, Any]]:
    latest_ack = latest_bot_issue_reply_time(comments)
    filtered: list[dict[str, Any]] = []
    for comment in comments:
        if is_bot_user(comment.get("user", {})):
            continue
        body = (comment.get("body") or "").strip()
        if is_bot_reply_body(body):
            continue
        if is_review_header_body(body):
            continue
        if REVIEW_TRIGGER_PHRASE in body:
            continue
        created_time = comment_time(comment)
        if (
...
def filter_review_issue_comments(
    comments: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    latest_ack = latest_bot_issue_reply_time(comments)
    filtered: list[dict[str, Any]] = []
    for comment in comments:
        body = (comment.get("body") or "").strip()
        if not is_review_header_body(body):
            continue
        created_time = comment_time(comment)
        if (
            latest_ack is not None
...
def latest_bot_reply_by_thread(
    comments: list[dict[str, Any]],
) -> dict[int, datetime]:
...
def filter_human_review_comments(
    ...
    for comment in comments:
        ...
        last_bot_reply = None
        if thread_root is not None:
            last_bot_reply = latest_bot_reply.get(thread_root)
        if last_bot_reply and created_time and created_time <= last_bot_reply:
            continue
        filtered.append(comment)
    return filtered
...
def is_blocking_review(
    ...
    if (
        user_login in REVIEW_BOT_LOGINS
    ...
    if user_login in REVIEW_BOT_LOGINS:
        return state == "CHANGES_REQUESTED"
    if body.startswith(BOT_REPLY_PREFIX) or state in ("APPROVED", "DISMISSED"):
        return False
    blocking = False
...
def raise_on_human_feedback(
    ...
    review_header_comments = filter_review_issue_comments(issue_comments)
    human_review_comments = filter_human_review_comments(review_comments)
    if human_issue_comments or human_review_comments or review_header_comments:
...
async def wait_for_review(pr_number: int, checks_done: asyncio.Event) -> None:
    ...
    while True:
        ...
        bot_issue_comments = filter_review_bot_comments(issue_comments, review_request_at)
        bot_review_comments = filter_review_bot_comments(review_comments, review_request_at)
        ...
        if bot_comments:
            ...
            if body:
                print("Reviewer left comments. Address feedback before merge.")
                print(body)
                raise SystemExit(2)
...
async def watch_pr() -> None:
    ...
    review_task = asyncio.create_task(wait_for_review(pr.number, checks_done))
    ...
    success_task = asyncio.gather(review_task, checks_task)
