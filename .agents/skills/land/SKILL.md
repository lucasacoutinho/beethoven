...
6. Ensure automated-review comments (if present) are acknowledged and any
   required fixes are handled before merging. See the *Review Handling*
   section below for the integration this project uses.
...
13. **Per-comment mode:** For each review comment, choose one of: accept,
    clarify, or push back. Reply inline (or in the issue thread for
    automated-review comments) stating the mode before changing code.
14. **Reply before change:** Always respond with intended action before pushing
    code changes (inline for review comments, issue thread for
...
# Wait for review feedback: automated reviews arrive as issue comments that
# start with the project's configured header (default this repo:
# "## Codex Review — <persona>"). Treat them like reviewer feedback: reply
# with a `[bot]` issue comment acknowledging the findings and whether you're
...
- Automated-review jobs retry on failure and are non-blocking; use the
  presence of issue comments matching the project's automated-review
  header pattern (default this repo: `## Codex Review — <persona>`) as the
  signal that review feedback is available, not the underlying job status.
...
- Automated reviews arrive as issue comments posted by a GitHub Action.
  They start with the project's configured header (default this repo:
  `## Codex Review — <persona>`) and include the reviewer's methodology +
  guardrails used. Treat these as feedback that must be acknowledged
  before merge.
...
- Use review comment endpoints (not issue comments) to find inline feedback:
  ...
  - Reply to a specific review comment:
    ...
    gh api -X POST /repos/{owner}/{repo}/pulls/<pr_number>/comments \
      -f body='[bot] <response>' -F in_reply_to=<comment_id>
    ```
...
- All GitHub comments generated by this agent must be prefixed with `[bot]`.
- For automated-review issue comments, reply in the issue thread (not a
  review thread) with `[bot]` and state whether you will address the
  feedback now or defer it (include rationale).
- If feedback requires changes:
  - For inline review comments (human), reply with intended fixes
    (`[bot] ...`) **as an inline reply to the original review comment** using
    the review comment endpoint and `in_reply_to` (do not use issue comments for
    this).
  - Implement fixes, commit, push.
  - Reply with the fix details and commit sha (`[bot] ...`) in the same place
    you acknowledged the feedback (issue comment for automated reviews, inline
    reply for review comments).
  - The land watcher treats automated-review issue comments as unresolved
    until a newer `[bot]` issue comment is posted acknowledging the findings.
- Only request a new automated review when you need a rerun (e.g., after
  new commits). Do not request one without changes since the last review.
  - Before requesting a new review, re-run the land watcher and ensure
    there are zero outstanding review comments (all have `[bot]` inline
    replies).
  - After pushing new commits, the automated-review workflow will rerun on
    PR synchronization (or you can re-run the workflow manually). Post a
    concise root-level summary comment so reviewers have the latest delta:
    ```
    [bot] Changes since last review:
  ...
  - Only request a new review if there is at least one new commit since the
    previous request.
  - Wait for the next automated-review comment before merging.
...
- If review feedback expands scope, decide whether to include it now or defer
  ...
  call it out in the root-level `[bot]` update with a brief reason (e.g.,
